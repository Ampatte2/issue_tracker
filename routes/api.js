...
const CONNECTION_STRING = "mongodb+srv://ampatte2:Roflpwn123@cluster0-vwsxg.mongodb.net/test?retryWrites=true&w=majority"; //MongoClient.connect(CONNECTION_STRING, function(err, db) {});
  ...
  app.route('/api/issues/:project')
    ...
    .get(function (req, res){
      ...
      MongoClient.connect(CONNECTION_STRING, function(err, db){
        ...
        collection.find(lookFor).toArray((err, docs)=>{
    ...
    .post(function (req, res){
      ...
      MongoClient.connect(CONNECTION_STRING, (err, db)=>{
                var database = db.db("test");
                var collection = database.collection("project");
                collection.insertOne(newEntry, (err, docs)=>{
                  if(err){res.json(err)};
                  res.json(docs.ops[0]);
                  db.close();
                }) 
    ...
    .put(function (req, res){
      ...
      MongoClient.connect(CONNECTION_STRING, function(err, db){
        var database = db.db("test");
        var collection = database.collection("project");
        collection.updateOne({_id: ObjectId(_id)}, {$set: updatedEntry}, (err, docs)=>{
          if (err){
            db.close();
            if(Object.keys(updatedEntry).length===0){return res.type("text").send("no updated field sent")}
          }
          if (docs.result.n === 0){
            res.type("text").send("could not update " + _id)
    ...
    .delete(function (req, res){
      ...
      MongoClient.connect(CONNECTION_STRING, function(err, db){
        ...
        var database = db.db("test");
        var collection = database.collection("project");
        collection.findOneAndDelete({_id: ObjectId(_id)}, (err, docs) =>{
          if (err){
            db.close();
          ...
          }else{res.type("text").send("delected " + _id)};
          db.close();
        })
      })
